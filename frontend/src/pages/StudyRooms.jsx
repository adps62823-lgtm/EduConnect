...
import { Button, EmptyState, Input, PageHeader, Spinner, Tag } from '@/components/ui'
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <div style={{ display: 'flex' }}>
          {(room.members || []).slice(0, 4).map((member, index) => (
            <div key={member.id} style={{ marginLeft: index > 0 ? -10 : 0, zIndex: 4 - index }}>
              <Avatar
                user={member}
                size="xs"
                showRing
      ...
      <div style={{ height: 4, background: 'var(--surface-3)', borderRadius: 2, overflow: 'hidden' }}>
        <motion.div
          ...
          style={{
            height: '100%',
            borderRadius: 2,
            background: isFull ? 'var(--red)' : pct > 75 ? 'var(--accent)' : 'var(--primary)',
          }}
        />
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '0.75rem', color: 'var(--text-3)' }}>
        ...
        <span>
          Host: <strong style={{ color: 'var(--text-2)' }}>{room.host?.name}</strong>
        </span>
...
function CreateRoomModal({ open, onClose, onCreated }) {
  ...
  const setField = (key) => (event) =>
    setForm((previous) => ({ ...previous, [key]: typeof event === 'string' ? event : event.target.value }))
  ...
  async function handleCreate() {
    ...
    if (form.is_private && !form.password.trim()) {
      toast.error('Set a password for private rooms.')
      return
    }
          ...
          <div
            ...
            onClick={() =>
              setForm((previous) => ({
                ...
                password: previous.is_private ? '' : previous.password,
              }))
            }
          >
...
export default function StudyRooms() {
  ...
  async function load() {
    ...
    try {
      ...
      setRooms(Array.isArray(allRooms) ? allRooms : allRooms?.rooms || [])
      setMyRooms(Array.isArray(memberRooms) ? memberRooms : memberRooms?.rooms || [])
    } catch {
      toast.error('Could not load study rooms.')
      setRooms([])
      setMyRooms([])
    } finally {
      setLoading(false)
  ...
  const joinedRoomIds = useMemo(() => new Set(myRooms.map((room) => room.id)), [myRooms])
  ...
  async function handleJoin(room) {
    ...
    try {
      ...
      toast.error(error.response?.data?.detail || 'Could not join room.')
    }
  }
  ...
  const displayed = (tab === 'mine' ? myRooms : rooms).filter((room) => {
    const matchQuery = !q || room.name.toLowerCase().includes(q.toLowerCase())
    const matchSubject = !subjectFilter || room.subject === subjectFilter
    const matchExam = !examFilter || room.exam_target === examFilter
    return matchQuery && matchSubject && matchExam
  })
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div>
        <div className="page-container">
          ...
          >
            {['all', 'mine'].map((nextTab) => (
              <button
                key={nextTab}
                onClick={() => setTab(nextTab)}
                className="profile-tab"
                style={{
                  color: tab === nextTab ? 'var(--primary)' : 'var(--text-3)',
                  borderBottomColor: tab === nextTab ? 'var(--primary)' : 'transparent',
                }}
              >
                {nextTab === 'all' ? 'All Rooms' : 'My Rooms'}
              </button>
            ))}
          </div>
