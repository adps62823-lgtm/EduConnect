...
export const profileAPI = {
  ...
  getResources:    (username, p) => api.get(`/profile/${username}/resources`, { params: p }),
  getBadges:       (username) => api.get(`/profile/${username}/badges`),

  // Exam countdowns
  addCountdown:    (d)   => api.post('/profile/exam-countdowns', d),
  getCountdowns:   ()    => api.get('/profile/exam-countdowns'),
  deleteCountdown: (id)  => api.delete(`/profile/exam-countdowns/${id}`),

  // Theme
  getTheme:        ()    => api.get('/profile/theme'),
  updateTheme:     (d)   => api.put('/profile/theme', d),
  uploadWallpaper: (file) => {
    const fd = new FormData(); fd.append('file', file)
    return api.post('/profile/theme/wallpaper', fd, {
      headers: { 'Content-Type': 'multipart/form-data' }
    })
  },
  getWallpapers:   ()    => api.get('/profile/theme/wallpapers'),
}
